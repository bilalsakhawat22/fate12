import React from 'react';

const SpecialItems = () => {
    return (
        <div className="home_our_special">
            <div className="container-JT">
                <div className="home_benifits_heading home_our_special_heading" data-aos="fade-up"
                     data-aos-duration="1000">
                    <h3>OUR SPECIAL <br/> LEGENDARY <br/> COLLECTION</h3>
                    <p>DISCOVER OUR 10 LEGENDARY Fate12 - THEY ARE THE RAREST</p>
                </div>
                <div className="home_our_special_flex" role="toolbar">
                            <div className="home_our_special_inner" tabIndex="-1"
                                 role="option" aria-describedby="slick-slide00" style={{width: 282}}
                                 data-slick-index="0" aria-hidden="false">
                                <div className="home_our_special_img">
                                    <img src="img/Legendary1.png" title="" alt=""/>
                                </div>
                                <div className="home_our_special_text">
                                    <h3>Golden Bunny</h3>
                                </div>
                            </div>
                            <div className="home_our_special_inner slick-slide slick-active" tabIndex="-1" role="option"
                                 aria-describedby="slick-slide01" style={{width: 282}} data-slick-index="1"
                                 aria-hidden="false">
                                <div className="home_our_special_img">
                                    <img src="img/Legendary2.png" title="" alt=""/>
                                </div>
                                <div className="home_our_special_text">
                                    <h3>Western Bunny</h3>
                                </div>
                            </div>
                            <div className="home_our_special_inner slick-slide slick-active" tabIndex="-1" role="option"
                                 aria-describedby="slick-slide02" style={{width: 282}} data-slick-index="2"
                                 aria-hidden="false">
                                <div className="home_our_special_img">
                                    <img src="img/Legendary3.png" title="" alt=""/>
                                </div>
                                <div className="home_our_special_text">
                                    <h3>Viking Bunny</h3>
                                </div>
                            </div>
                            <div className="home_our_special_inner slick-slide slick-active" tabIndex="-1" role="option"
                                 aria-describedby="slick-slide03" style={{width: 282}} data-slick-index="3"
                                 aria-hidden="false">
                                <div className="home_our_special_img">
                                    <img src="img/Legendary4.png" title="" alt=""/>
                                </div>
                                <div className="home_our_special_text">
                                    <h3>Knight Bunny</h3>
                                </div>
                            </div>
                            <div className="home_our_special_inner slick-slide slick-active" tabIndex="-1" role="option"
                                 aria-describedby="slick-slide04" style={{width: 282}} data-slick-index="4"
                                 aria-hidden="false">
                                <div className="home_our_special_img">
                                    <img src="img/Legendary5.png" title="" alt=""/>
                                </div>
                                <div className="home_our_special_text">
                                    <h3>OG Gold Bunny</h3>
                                </div>
                            </div>
                            <div className="home_our_special_inner slick-slide" tabIndex="-1" role="option"
                                 aria-describedby="slick-slide05" style={{width: 282}} data-slick-index="5"
                                 aria-hidden="true">
                                <div className="home_our_special_img">
                                    <img src="img/Legendary6.png" title="" alt=""/>
                                </div>
                                <div className="home_our_special_text">
                                    <h3>Pilot Bunny</h3>
                                </div>
                            </div>
                            <div className="home_our_special_inner slick-slide" tabIndex="-1" role="option"
                                 aria-describedby="slick-slide06" style={{width: 282}} data-slick-index="6"
                                 aria-hidden="true">
                                <div className="home_our_special_img">
                                    <img src="img/Legendary7.png" title="" alt=""/>
                                </div>
                                <div className="home_our_special_text">
                                    <h3>Santa Gold Bunny</h3>
                                </div>
                            </div>
                            <div className="home_our_special_inner slick-slide" tabIndex="-1" role="option"
                                 aria-describedby="slick-slide07" style={{width: 282}} data-slick-index="7"
                                 aria-hidden="true">
                                <div className="home_our_special_img">
                                    <img src="img/Legendary8.png" title="" alt=""/>
                                </div>
                                <div className="home_our_special_text">
                                    <h3>Don Bunny</h3>
                                </div>
                            </div>
                            <div className="home_our_special_inner slick-slide" tabIndex="-1" role="option"
                                 aria-describedby="slick-slide08" style={{width: 282}} data-slick-index="8"
                                 aria-hidden="true">
                                <div className="home_our_special_img">
                                    <img src="img/Legendary9.png" title="" alt=""/>
                                </div>
                                <div className="home_our_special_text">
                                    <h3>Sage Bunny</h3>
                                </div>
                            </div>
                            <div className="home_our_special_inner slick-slide" tabIndex="-1" role="option"
                                 aria-describedby="slick-slide09" style={{width: 282}} data-slick-index="9"
                                 aria-hidden="true">
                                <div className="home_our_special_img">
                                    <img src="img/Legendary10.png" title="" alt=""/>
                                </div>
                                <div className="home_our_special_text">
                                    <h3>Prince Bunny</h3>
                                </div>
                            </div>
                        </div>
            </div>
        </div>
    );
}

export default SpecialItems